...
void  Game::createBullet(const b2Vec2& vpos, const b2Vec2& vdir)
{
  ...
  bd.position = vpos;
  bd.bullet = true;
  bd.linearVelocity = s2b*300.0f * vdir;  //prêdkoœæ pocisku 300pix/s
...
void  Game::createAsteroid(AsteroidType type)
{
  ...
  bd.linearVelocity *= s2b * 80;  //80m/s 
  obj->body = b2world->CreateBody(&bd);
...
void  Game::userInput(const Input& in)
{
  ...
  if(in.kb[VK_UP] == Input::Hold) //przyœpieszanie w przód
  {
  ...
  if(in.kb[VK_DOWN] == Input::Hold)//przyœpieszanie w ty³
  {
    b2Vec2 vdir(starship.body->GetTransform().q.c, starship.body->GetTransform().q.s);
...
void  Game::process(const Input& in)
{
  ...
  //asteroidy
  if((++timer % 60*5) == 0 && vasteroids.size() < 5) // co 5s tworzymy nowy asteroid, max 5 asteroidow
  {
  ...
  for(size_t a = 0; a < vasteroids.size(); a++)
  {
    ...
    if(fabs(vpos.x) > s2b*win_dx/2 || fabs(vpos.y) > s2b*win_dy/2)
    {
      ...
      vdir.Normalize();
      vdir *= 200.0f;
      vasteroids[a]->body->SetLinearVelocity(s2b*vdir);//  ApplyForceToCenter(vdir, true);
    }
  }
  ...
  for(size_t q = 0; q<vbullets.size();)
  {
    ...
    for(b2ContactEdge* ce = vbullets[q]->body->GetContactList(); ce != NULL; ce = ce->next)
    {
      ...
      if(ce->contact->IsTouching())
      {
        ...
        if((filterA.categoryBits | filterB.categoryBits) & Filters::Bit_Asteroid_Core) //kolizja z asteroid core
        {
        ...
        if((filterA.categoryBits | filterB.categoryBits) & Filters::Bit_Asteroid_Shield) //kolizja z asteroid shield
        {
          //partikle
          //w manifoldzie w polu points jest pozycja kontaktu, bêdzie to pozycja emitera cz¹stek
          b2WorldManifold wm;
